...
interface DailyGroupMetricFormData {
...
export function DailyGroupMetrics() {
  const [metrics, setMetrics] = useState<DailyGroupMetric[]>([]);
  const [loading, setLoading] = useState(true);
  const [searchTerm, setSearchTerm] = useState('');
  const [isDialogOpen, setIsDialogOpen] = useState(false);
  const [editingMetric, setEditingMetric] = useState<DailyGroupMetric | null>(null);
  const [deletingMetric, setDeletingMetric] = useState<DailyGroupMetric | null>(null);
  const [submitting, setSubmitting] = useState(false);
  const [formData, setFormData] = useState<DailyGroupMetricFormData>({
    snapshotId: null,
    metricName: '',
    aggType: '',
    value: '',
  ...
  const loadMetrics = async () => {
    try {
      setLoading(true);
      const data = await fetchAPI('/daily-group-metrics');
      setMetrics(data);
    } catch (err: any) {
  ...
  const handleOpenDialog = (metric?: DailyGroupMetric) => {
    if (metric) {
      ...
      setFormData({ snapshotId: null, metricName: '', aggType: '', value: '' });
    }
  ...
  const handleSubmit = async () => {
    if (!formData.snapshotId || !formData.metricName || !formData.aggType) {
    ...
    try {
      if (editingMetric) {
        await fetchAPI(
          `/daily-group-metrics/${editingMetric.snapshotId}/${editingMetric.metricName}/${editingMetric.aggType}`,
          { method: 'PUT', body: JSON.stringify({ ...formData, value: Number(formData.value) }) }
        );
        toast.success('Métrica actualizada');
      } else {
        await fetchAPI('/daily-group-metrics', { method: 'POST', body: JSON.stringify({ ...formData, value: Number(formData.value) }) });
  ...
  const handleDelete = async () => {
    ...
    try {
      await fetchAPI(
        `/daily-group-metrics/${deletingMetric.snapshotId}/${deletingMetric.metricName}/${deletingMetric.aggType}`,
        { method: 'DELETE' }
      );
  ...
  const filteredMetrics = metrics.filter(m =>
    ...
    m.snapshot?.date.includes(searchTerm)
  );

  if (loading) return (
    <div className="flex flex-col justify-center items-center h-64 space-y-4">
      <Loader2 className="w-12 h-12 animate-spin text-orange-500" />
      <p className="text-gray-400">Cargando métricas de grupo...</p>
    </div>
  );
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          <h3 className="text-2xl font-bold text-white flex items-center space-x-2">
            ...
            <span>Métricas Diarias de Grupo</span>
          </h3>
          <p className="text-gray-400 text-sm mt-1">
            Administra métricas por snapshot y tipo
          </p>
        </div>
      ...
      <Card className="border-white/10 bg-slate-900/30 backdrop-blur-sm">
        <CardContent>
          <div className="relative">
            ...
            <Input
              placeholder="Buscar por snapshot, métrica o agregación..."
              value={searchTerm}
      ...
      <div className="space-y-4">
        {filteredMetrics.length === 0 ? (
          ...
          filteredMetrics.map(metric => (
            <Card key={`${metric.snapshotId}-${metric.metricName}-${metric.aggType}`} className="bg-slate-800/50 border-white/10 flex justify-between items-center p-4">
              <div>
                <p className="text-white font-semibold">{metric.metricName} [{metric.aggType}] = {metric.value}</p>
      ...
      <Dialog open={isDialogOpen} onOpenChange={setIsDialogOpen}>
        <DialogContent className="sm:max-w-lg bg-slate-900 border-white/10 text-white">
          <DialogHeader>
            ...
            <DialogDescription>Completa los datos del snapshot y la métrica</DialogDescription>
          </DialogHeader>
      ...
      <Dialog open={!!deletingMetric} onOpenChange={() => setDeletingMetric(null)}>
        <DialogContent className="sm:max-w-md bg-slate-900 border-white/10 text-white">
          ...
          <p className="py-4 text-gray-300">¿Deseas eliminar la métrica {deletingMetric?.metricName} [{deletingMetric?.aggType}]?</p>
          <DialogFooter className="flex gap-2">
