import { Controller, Post, Body, Get, Param, Put, Delete, UseGuards } from '@nestjs/common';
import { AdminSubdivisionsService } from './admin-subdivisions.service';
import { CreateAdminSubdivisionDto } from './dto/create-admin-subdivision.dto';
import { UpdateAdminSubdivisionDto } from './dto/update-admin-subdivision.dto';
import { JwtRedisGuard } from '../../auth/jwt-redis.guard';
import { RolesGuard } from '../../auth/roles.guard';
import { Roles } from '../../auth/roles.decorator';

@Controller('admin-subdivisions')
@UseGuards(JwtRedisGuard, RolesGuard)
export class AdminSubdivisionsController {
  constructor(private readonly service: AdminSubdivisionsService) {}

  @Post()
  @Roles('Admin')
  create(@Body() dto: CreateAdminSubdivisionDto) {
    return this.service.create(dto);
  }

  @Get()
  @Roles('Admin', 'Manager')
  findAll() {
    return this.service.findAll();
  }

  @Get(':id')
  @Roles('Admin', 'Manager')
  findOne(@Param('id') id: number) {
    return this.service.findOne(+id);
  }

  @Put(':id')
  @Roles('Admin')
  update(@Param('id') id: number, @Body() dto: UpdateAdminSubdivisionDto) {
    return this.service.update(+id, dto);
  }

  @Delete(':id')
  @Roles('Admin')
  remove(@Param('id') id: number) {
    return this.service.remove(+id);
  }
}
