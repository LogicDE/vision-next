import { Controller, Post, Body, Get, Param, Put, Delete, UseGuards } from '@nestjs/common';
import { GroupSnapshotsService } from './group-snapshots.service';
import { CreateGroupSnapshotDto } from './dto/create-group-snapshot.dto';
import { UpdateGroupSnapshotDto } from './dto/update-group-snapshot.dto';
import { JwtRedisGuard } from '../../auth/jwt-redis.guard';
import { RolesGuard } from '../../auth/roles.guard';
import { Roles } from '../../auth/roles.decorator';

@Controller('group-snapshots')
@UseGuards(JwtRedisGuard, RolesGuard)
export class GroupSnapshotsController {
  constructor(private readonly service: GroupSnapshotsService) {}

  @Post()
  @Roles('Admin', 'Manager')
  create(@Body() dto: CreateGroupSnapshotDto) {
    return this.service.create(dto);
  }

  @Get()
  @Roles('Admin', 'Manager')
  findAll() {
    return this.service.findAll();
  }

  @Get(':id')
  @Roles('Admin', 'Manager')
  findOne(@Param('id') id: number) {
    return this.service.findOne(+id);
  }

  @Put(':id')
  @Roles('Admin', 'Manager')
  update(@Param('id') id: number, @Body() dto: UpdateGroupSnapshotDto) {
    return this.service.update(+id, dto);
  }

  @Delete(':id')
  @Roles('Admin', 'Manager')
  remove(@Param('id') id: number) {
    return this.service.remove(+id);
  }
}
